          ...
          <div class="flex items-center justify-center flex-wrap gap-0">
            <template v-for="(stage, idx) in route" :key="idx">
              ...
              <div v-if="idx > 0" class="flex flex-col items-center mx-1 sm:mx-2">
                <span class="text-xs sm:text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap font-medium">{{ stage.evolve_level ? 'Lv.' + stage.evolve_level : (formatEvoCondition(stage.evolve_condition) || '特殊') }}</span>
                <svg class="w-5 h-5 sm:w-6 sm:h-6 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6" />
                </svg>
                <span v-if="stage.evolve_level && stage.evolve_condition" class="text-[10px] text-muted whitespace-nowrap max-w-[100px] truncate" :title="formatEvoCondition(stage.evolve_condition)">{{ formatEvoCondition(stage.evolve_condition) }}</span>
              </div>
              <!-- Pet stage card -->
