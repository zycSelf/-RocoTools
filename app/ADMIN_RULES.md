...
所有属性相关的展示（筛选按钮、下拉选择、属性列表等）必须严格按照以下固定顺序：

```
普通 → 草 → 火 → 水 → 光 → 机械 → 地 → 冰 → 龙 → 电 → 毒 → 虫 → 武 → 翼 → 萌 → 幽 → 恶 → 幻
```

实现方式：`app/server/src/constants/elementOrder.js` 定义 `ELEMENT_SORT_ORDER` 映射（element_id → sort_position），`elements` API 返回时在 JS 层排序。

### 技能排序规则

技能展示的排序优先级为：**学习等级 > 属性顺序 > 技能ID**

| 场景 | 排序规则 | 说明 |
|------|----------|------|
| 精灵升级技能（skills） | 等级 → 属性 → 技能ID | 先按学习等级升序，同等级内按属性顺序，同属性按技能ID数字升序 |
| 精灵血脉技能（bloodline_skills） | 属性 → 技能ID | 按属性顺序排序，同属性按技能ID数字升序 |
| 精灵技能石（learnable_stones） | 属性 → 技能ID | 按属性顺序排序，同属性按技能ID数字升序 |
| 技能列表页 | 属性 → 技能ID | 按属性顺序排序，同属性按技能ID数字升序 |
| 技能选择弹窗（选中属性时） | 技能ID | 属性固定时直接按技能ID数字升序 |

### 技术实现

- 属性排序使用 SQL `CASE WHEN` 表达式：`elementOrderSql('column_name')`
